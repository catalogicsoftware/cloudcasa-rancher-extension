...
export default {
  ...
  methods: {
    ...
    async getCloudCasaData(clusterName){
      ...
      networkRequest.url = networkRequest.url + `kubeclusters?where={
    ...
    async getCloudCasaBackupData(cloudCasaClusterId){
      let networkRequest = await getCloudCasaRequest(this.$store);
      networkRequest.method = 'GET';
      networkRequest.url = networkRequest.url + `kubebackups?sort=-_updated&
        embedded={"policy": 1, "copy_policy": 1, "cluster": 1}&where={
          "migrationdef":{"$exists": false},
    ...
    parseCloudCasaJobData(type, rawData){
      for (let i = 0; i < rawData._items.length; i++){
        ...
        let parsedDate = '';
        if (rawData._items[i].status.last_job_run_time != undefined) {
